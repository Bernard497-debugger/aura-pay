...
<head>
    ...
    <style>
        ...
        .amount-input { background: transparent; border: none; color: white; font-size: 3.5rem; width: 100%; text-align: center; outline: none; margin: 20px 0; font-weight: 800; }
        .mode-toggle { display: flex; gap: 10px; margin-bottom: 20px; }
        .mode-btn { flex: 1; padding: 10px; border-radius: 12px; border: 1px solid #333; background: #1a1a1a; color: white; cursor: pointer; }
...
<body>
    <div class="card">
        ...
        <p id="status-label" style="font-size: 12px; opacity: 0.5;">SECURE TERMINAL</p>

        <input type="number" id="amount" class="amount-input" value="10.00" step="0.01">
    ...
    <script>
        ...
        function setMode(newMode) {
            ...
            document.getElementById('status-label').innerText = mode.toUpperCase() + " MODE";
        }
        ...
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                ...
                return fetch(url, { method: 'POST' })
                    ...
                    .then(order => {
                        ...
                        alert("Error creating order. Check logs.");
                    });
            },
            onApprove: function(data, actions) {
                return fetch('/capture/' + data.orderID, { method: 'POST' })
                    .then(res => res.json())
                    .then(details => {
                        alert('Transaction Successful! ID: ' + details.id);
                        location.reload();
                    });
            },
            onError: function(err) {
                console.error('PayPal SDK Error:', err);
                alert("PayPal failed to load. Are your Live Keys correct?");
            }
...
def create_order():
    ...
    amount = request.args.get('amt', '10.00')
    payee_email = request.args.get('to') # Optional
    ...
    # Only add payee if we are in 'Send' mode
    if payee_email:
